...
def generate_demo_plot():
    ...
    try:
        ...
        print("Could not load model.pth, please be sure it is trained.")
        return
    ...
    ax2.scatter(gen_pts[:, 0].numpy(), gen_pts[:, 1].numpy(), gen_pts[:, 2].numpy(), c='r', alpha=0.6)
    ...
    ax3.hist(gen_dists.numpy(), bins=30, alpha=0.5, label="Generated", color='r')
